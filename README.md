...
**聊天页**（终端风）
- Ollama 模型流式对话（多轮上下文、停止生成、清空、系统提示词）
- **[SIoT] 按钮**：采集 SIoT 实时数据并插入对话，让 AI 能"读到"传感器数据（如问"现在温度多少"）——采集时长为 3/5/10 秒，topic 有历史记忆
- ⚙ 设置含 Ollama 与 SIoT（MQTT）两组参数（与仪表盘共享配置）

**仪表盘**（监控大屏风）
- 6 种 ECharts 图表（折线/面积/柱状/饼图/仪表盘/散点）+ **数值展示**（大字号最新值+单位）+ **控制指令**（向 topic 下发 on/off 等，带确认）+ **图传画面**（base64 图片实时渲染）
- **阈值告警**：数值类卡片可设上下限，超限卡片变红 + 呼吸告警徽标
- 全局连接状态指示灯、卡片增删改、配置持久化（localStorage）

**首页**：Ollama / SIoT 在线状态预览 + 导航
